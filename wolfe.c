...
static int align_template(Example *e, Tokens *q, Alignment *a, Tool *tool) {
    ...
    cur.first = -1;
    best.score = -1e30;
    for (i = 0; i < NW; i++) cur.begin[i] = cur.end[i] = -1;
    ...
    for (i = 0; i < e->tokens.n; i++) if (e->tokens.t[i].slot) {
        ...
        if (pi >= 0) {
            ...
            a->end[pi] = best.end[i];
            if (tool->p[pi].required && best.begin[i] == best.end[i]) a->score -= 3.;
        }
    }
    return 1;
}
...
static void string_span(const char *input, Tokens *q, Tool *tool, int property, int *begin, int *end,
    int *start, int *finish) {
    int i, k, en;
    while (*begin < *end && (!strcmp(q->t[*begin].word, "in") || !strcmp(q->t[*begin].word, "for") || !strcmp(q->t[*begin].word,
        "to")))(*begin)++;
    while (*end > *begin && (!strcmp(q->t[*end - 1].word, "please") || !strcmp(q->t[*end - 1].word, "kindly")))(*end)--;
    ...
    for (i = *begin; i < *end; i++) for (k = 0; k < tool->np; k++) if (k != property) {
        Property *p = &tool->p[k];
        int v;
        for (v = 0; v < p->nv; v++) if (phrase_at(q, i, p->values[v].value, &en)) {
            * end = i;
  ...
  * span wins over an introduction such as 'the track' or 'saying'. */
    {
        ...
        if (a >= 0 && b > a && (a == *start || !strncmp(input + *start, "the ", 4) || !strncmp(input + *start,
            "saying ", 7) || !strncmp(input + *start, "called ", 7) || !strncmp(input + *start, "named ",
            6))) {
            * start = a + 1;
            *finish = b;
            return;
...
static int extract(Model *m, const char *input, Result *r) {
    ...
    for (p = 0; p < tool->np; p++) {
        ...
        } else if (best_e >= 0 && best.begin[p] >= 0 && begin < end) {
            int start, finish;
            char raw[NS];
            string_span(input, &r->input, tool, p, &begin, &end, &start, &finish);
            if (finish > start && finish - start < NS) {
                memcpy(raw, input + start, (size_t)(finish - start));
                raw[finish - start] = 0;
                if (quote_raw(raw, c->raw, sizeof c->raw)) {
                    c->present = 1;
                    at = begin;
                    until = end;
                    c->start = start;
        ...
        if (c->present && pr->type == PSTR && !pr->nv && best_e >= 0) {
            int exi;
            for (exi = 0; exi < m->ne; exi++) if (m->ex[exi].tool == r->winner) {
                Tokens *pattern = &m->ex[exi].tokens;
                int z, slots = 0, last = -1;
                for (z = 0; z < pattern->n; z++) {
                    if (pattern->t[z].slot) slots++;
                    else if (!filler(pattern->t[z].word) && strcmp(pattern->t[z].word, "by")
                        && strcmp(pattern->t[z].word, "from") && strcmp(pattern->t[z].word, "in")
                        && strcmp(pattern->t[z].word, "at") && strcmp(pattern->t[z].word, "on")) last = z;
                }
                if (slots && last >= 0 && end > begin && end - begin <= 2 && !strcmp(pattern->t[last].word,
                    r->input.t[end - 1].word) && !r->input.t[end - 1].quoted) c->present = 0;
            }
        }
        if (c->present) {
   ...
   * the explicit way to supply an absent value. */
        ...
        if (!c->present && pr->required) return 0;
    }
    return 1;
}
...
static void output(Model *m, const char *text, Result *r, int mode, int reasoning) {
    ...
    if (r->status == CALL) {
        ...
        fputs("{\"name\":", stdout);
        emit_string(stdout, t->name);
        fputs(",\"arguments\":{", stdout);
        j = 0;
        for (i = 0; i < t->np; i++) if (r->arg[i].present) {
            if (j++) putchar(',');
            emit_string(stdout, t->p[i].name);
            putchar(':');
            fputs(r->arg[i].raw, stdout);
        }
        fputs("}}", stdout);
    }
    fputs("],\"status\":", stdout);
    emit_string(stdout, status_name(r->status));
    printf(",\"confidence\":%.6f", r->confidence);
    if (r->status == MISSING) {
        Tool *t = &m->tool[r->winner];
        fputs(",\"tool\":", stdout);
        emit_string(stdout, t->name);
    ...
    if (reasoning) {
        fputs(",\"reasoning\":{\"mode\":", stdout);
        emit_string(stdout, mode == 0 ? "neural" : mode == 1 ? "field" : "keyword");
        printf(",\"iterations\":%d,\"margin\":%.6f,\"score_kind\":\"activation, not calibrated probability\",\"candidates\":[",
            r->iterations, r->margin);
        for (i = 0; i <= m->nt; i++) {
            if (i) putchar(',');
            fputs("{\"tool\":", stdout);
            if (i == m->nt) fputs("null", stdout);
            else emit_string(stdout, m->tool[i].name);
            printf(",\"score\":%.6f,\"field\":%.6f,\"prophecy\":%.6f", selected_score(&r->c[i], mode),
                r->c[i].field, r->c[i].prophecy);
            if (r->c[i].best >= 0) {
                fputs(",\"example\":", stdout);
                emit_string(stdout, m->ex[r->c[i].best].text);
            }
            putchar('}');
        }
        putchar(']');
        ...
        if (r->winner >= 0 && r->c[r->winner].best >= 0) {
            ...
            for (i = 0; i < r->input.n; i++) {
                ...
                if (strength > .30 && !filler(r->input.t[i].word)) {
                    if (j++) putchar(',');
                    fputs("{\"text\":", stdout);
                    emit_string(stdout, r->input.t[i].word);
                    fputs(",\"relation\":", stdout);
                    emit_string(stdout, related);
                    printf(",\"strength\":%.6f,\"span\":[%d,%d]}", strength, r->input.t[i].start, r->input.t[i].end);
                }
            }
        }
        putchar(']');
        ...
        if (r->winner >= 0 && r->winner < m->nt) for (i = 0; i < m->tool[r->winner].np; i++) if (r->arg[i].present) {
            ...
            if (j++) putchar(',');
            fputs("{\"name\":", stdout);
            emit_string(stdout, m->tool[r->winner].p[i].name);
            fputs(",\"value\":", stdout);
            fputs(c->raw, stdout);
            fputs(",\"source\":", stdout);
            emit_string(stdout, c->source);
            printf(",\"span\":[%d,%d]}", c->start, c->end);
        }
        putchar(']');
        fputs(",\"trajectory\":[", stdout);
        for (i = 0; i < r->iterations; i++) {
            if (i) putchar(',');
            printf("%.6f", r->trajectory[i][r->winner]);
        }
        fputs("]}", stdout);
    }
    fputs("}\n", stdout);
...
static int save_state(Model *m, const char *path) {
    ...
    fprintf(f, "{\"version\":1,\"identity\":\"%016llx\",\"tools\":{", (unsigned long long) m->identity);
    for (i = 0; i < m->nt; i++) {
        if (i) fputc(',', f);
...
int main(int argc, char * *argv) {
    ...
    for (i = 1; i < argc; i++) {
        if (!strcmp(argv[i], "--help") || !strcmp(argv[i], "-h")) {
            ...
            || !strcmp(argv[i], "--feedback") || !strcmp(argv[i], "--mode")) {
    ...
    if (!load_tools(m, tools) || !load_examples(m, examples) || !build_field(m) || !load_state(m, state)) {
